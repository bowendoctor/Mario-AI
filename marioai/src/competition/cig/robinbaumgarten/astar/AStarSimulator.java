...
public class AStarSimulator 
{
    ...
    */
    ...
    private void search(long startTime,SearchNode current)
	{
		//System.out.println(bestPosition!=null);
    	//boolean currentGood = false;		// is the current node good (= we're not getting hurt)
    	//int ticks = 0;
    	int maxRight = 40;					// distance to plan to the right
		if(current==null)
			return;
		try {
			/*
			...
			if(levelScene.level.isGap[(int) (current.sceneSnapshot.mario.x / 16)])
				System.out.println("4");
			if(System.currentTimeMillis() - startTime >= 40)
				System.out.println("1");
				*/
			if ((System.currentTimeMillis() - startTime >= 200) || (levelScene.level.isGap[(int) (current.sceneSnapshot.mario.x / 16)]) || (bestPosition!=null)) {
				return;
			}
			if(current.parentPos.sceneSnapshot.mario.x==current.sceneSnapshot.mario.x&&current.parentPos.sceneSnapshot.mario.y==current.sceneSnapshot.mario.y) {
				//System.out.println("2");
				return;
			}
			if (current.sceneSnapshot.mario.x - currentSearchStartingMarioXPos >= maxRight) {
		...
		ArrayList<SearchNode> child=new ArrayList<SearchNode>();
		child=current.generateChildren();
		for (SearchNode a: child) {
			a.simulatePos();
			search(startTime, a);
		}
    ...
    private void startSearch(long startTime,int repetitions)
    {    	
    	...
    	*/
    	...
    	bestPosition = null;
    	search(startTime,startPos);
		if(bestPosition!=null)
			levelScene = bestPosition.sceneSnapshot;
    }
	...
	public boolean[] optimise()
	{
        ...
        if (ticksBeforeReplanning <= 0 || currentActionPlan.size() == 0)
        {
        	...
        	startSearch(startTime,stepsPerSearch);
